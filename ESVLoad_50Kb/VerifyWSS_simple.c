VerifyWSS_simple()
{
	...
	if(strstr( lr_eval_string("{result}"), lr_eval_string("{VerifyWSS_simple_response}"))==NULL)
	{
        lr_end_transaction("VerifyWSS_simple", LR_FAIL);
		lr_output_message( lr_eval_string("error: expected contains {VerifyWSS_simple_response}, but got {result}"));
		return -1;
	}

    lr_end_transaction("VerifyWSS_simple", LR_PASS);
	return 0;
}
